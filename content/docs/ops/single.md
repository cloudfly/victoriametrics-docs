---
title: 单机版本
date: 2024-11-06T19:05:45+08:00
...
VictoriaMetrics 的容量与可用资源呈线性关系。所需的 CPU 和 RAM 数量高度依赖于数据量 - [活跃时间序列]({{< relref "../faq.md#what-is-an-active-time-series" >}})的数量、指标[替换率]({{< relref "../faq.md#what-is-high-churn-rate" >}})、查询类型、每秒请求数等等。

建议根据[故障排除](#troubleshooting)文档，为你的生产数据搭建一个测试 VictoriaMetrics，然后反复调整 CPU 和 RAM 资源，直到它稳定运行。根据我们的[案例研究](https://docs.victoriametrics.com/CaseStudies.html)，单机版 VictoriaMetrics 可以完美处理以下数据量：
...
建议预留如下资源：
+ 为了应对突发流量，避免内存溢出（OOM）导致系统崩溃或性能下降，建议保留`50%`的空闲内存。
+ 为了应对突发流量，出现系统可用性下降，将`50%`的空闲 CPU。
+ 为`-storageDataPath`参数指定的目录保留至少 [20% 的可用存储空间](#storage)。详见[此处](#flags)`-storage.minFreeDiskSpaceBytes`启动参数说明。

参见[资源使用限制](#limitation)。

### 保存时间(期限) {#retention}

保存期限是通过`-retentionPeriod`启动参数指定的，该参数后面跟着一个数字和时间单位字符 - `h`(小时),`d`(天),`w`(周),`y`(年)。如果未指定时间单位，则默认为**月份**。例如，`-retentionPeriod=3`表示数据将被存储`3`个月然后删除。**默认的保留期为一个月**。最小的保留期为`24`小时或者`1`天。

系统将数据分割成每月的分区(`partition`)，存储在`<-storageDataPath>/data/{small,big}`文件夹中。每个`partition`由一个或多个`part`组成。

**超出保存期限的数据分区会在新月份的第一天被删除。超出保存期限的`part`最终会在后台合并过程中删除。**

`part`所覆盖的时间范围不受保存期限限制。一个数据`part`可以涵盖几小时或几天的数据。因此，只有当完全超出保存期限时才能删除这个`part`。

设定的保存期限（`-retentionPeriod`）对应的最大磁盘空间使用量将是`-retentionPeriod`+ 1 个月。例如，如果`-retentionPeriod`设置为`1`，则一月份的数据将在3月1日被删除。

在现有数据上延长保存时间是安全的。如果将保存时间（`-retentionPeriod`）设置为比之前更低的值，则超过保存时间外的数据最终将被删除。

VictoriaMetrics 不支持无限保存时间，但您可以指定一个任意长的持续时间，例如`-retentionPeriod=100y`。

### 资源使用限制 {#limitation}

默认情况下，VictoriaMetrics 针对典型业务场景进行了优化，追求最大化利用计算资源。某些业务场景可能需要更细致地调整资源使用。在这些情况下，下面这些启动参数可能会有用：

+ `-memory.allowedPercent`和`-memory.allowedBytes`：限制 VictoriaMetrics 内部缓存使用的内存大小。请注意，这不代表 VictoriaMetrics 进程的内存使用限制，因为这些参数不限制每个查询执行时所消耗的内存。
+ `-search.maxMemoryPerQuery`：限制用于处理单个查询的内存用量。需要更多内存的查询会报错。查询大量数据的重查询可能会稍微超过每个查询的内存限制。并行的查询的总内存限制可以估计为`-search.maxMemoryPerQuery * -search.maxConcurrentRequests`。
+ `-search.maxUniqueTimeseries`：限制单次查询允许检索并处理的独立时间序列的数量。VictoriaMetrics 在内存中保留有关每个查询检索到的时间序列的一些元信息，并花费一些CPU时间来处理检索的时间序列。这意味着单个查询可以使用的最大内存使用量和CPU使用量与`-search.maxUniqueTimeseries`成比例。
...
+ `-search.maxTagKeys`限制从 [/api/v1/labels]({{< relref "../query/api.md#apiv1labels" >}}) 返回的数据条数。该接口主要用于 Grafana 实现 Label Name 自动提示。当数据库包含大量独立时间序列时，对该接口的查询可能会消耗大量的 CPU 和内存，因为存在[高替换率]({{< relref "../faq.md#what-is-high-churn-rate" >}})。在这种情况下，将`-search.maxTagKeys`设置为较低值有助于限制 CPU 和内存消耗。
+ `-search.maxTagValues`限制从 [/api/v1/label/.../values]({{< relref "../query/api.md#apiv1labelnamevalues" >}}) 返回的数据条数。该接口主要用于 Grafana 实现 Label Value 自动提示。由于[高替换率]({{< relref "../faq.md#what-is-high-churn-rate" >}})的原因，当数据库包含大量独立时间序列时，对该接口的查询可能会消耗大量 CPU 时间和内存。在这种情况下，将`-search.maxTagValues`设置为较低的值有助于限制 CPU 和内存消耗。
+ `-search.maxTagValueSuffixesPerSearch`限制了从`/metrics/find`接口返回的数据条数。请参阅 [Graphite Metrics API使用文档](https://docs.victoriametrics.com/Single-server-VictoriaMetrics.html#graphite-metrics-api-usage)。
...
1. 在不同的数据中心（available zone）安装多个 VictoriaMetrics 实例。
2. 将这些实例的地址通过`-remoteWrite.url`启动参数传递给 [vmagent](https://docs.victoriametrics.com/vmagent.html)。
...
或者这些地址可以写在 Prometheus 启动配置的`remote_write`部分：
...
+ 现在 Prometheus 应该并行地将数据写入所有配置的`remote_write`地址。
+ 在所有的 VictoriaMetrics 实例前面设置 [Proxy](https://github.com/jacksontj/promxy)。
+ 在 Grafana 中设置一个指向 Proxy 的 Prometheus 数据源。

#### 关于数据重复
如果你的 Prometheus 本身也是多实例的，比如 2 个完全一样的 Prometheus（`r1`和`r2`） 做一模一样的事。那么请将`r1`配置为将数据写入`victoriametrics-addr-1`，而`r2`将数据写入`victoriametrics-addr-2`。

当然你也可以让`r1`和`r2`的 remote write 配置完全一样，二者都是双发的，VictoriaMetrics 会自动对相同的数据进行[去重](#deduplicate)。

### 监控 {#metrics}
VictoriaMetrics 在`/metrics`地址上以 Prometheus Text 格式暴露内部指标。这些指标可以使用 [vmagent](https://docs.victoriametrics.com/vmagent.html) 或 Prometheus 进行采集。或者，当`-selfScrapeInterval`启动参数设置为大于 0 时，单机版的 VictoriaMetrics 可以自动采集指标。例如，`-selfScrapeInterval=10s`将启用每 10 秒自动采集一次`/metrics`地址。

官方提供了适用于[单机版](https://grafana.com/grafana/dashboards/10229-victoriametrics/)和[集群版](https://grafana.com/grafana/dashboards/11176-victoriametrics-cluster/) VictoriaMetrics 的 Grafana 监控面板。也可以使用由社区创建的适用于[集群 VictoriaMetrics 的监控面板](https://grafana.com/grafana/dashboards/11831)。

监控面板上的图表包含有用的提示，可以将鼠标悬停在每个图表左上角的`i`图标上查看指标介绍。

我们建议使用 [vmalert]({{< relref "../components/vmalert.md" >}}) 或 Prometheus 设置警报。

VictoriaMetrics 在`/api/v1/status/active_queries`地址中展示当前正在执行的查询以及它们的运行时间，在`/api/v1/status/top_queries`地址中展示执行时间最长的查询语句。
...
#### Push Metrics
所有的 VictoriaMetrics 组件支持将`/metrics`地址上开放的指标以 Prometheus Text Format 协议推送到其他地方。如果 VictoriaMetrics 组件位于隔离网络中，无法被本地 [vmagent]({{< relref "../components//vmagent.md" >}})采集，则可以使用此功能来替代[经典的类 Prometheus 指标抓取]({{< relref "../write/model.md#pull" >}})。

使用下面的启动参数可以将组件的指标 Push 到指定地址：

+ `-pushmetrics.url`: Push 的目标 URL 地址。比如，`-pushmetrics.url=http://victoria-metrics:8428/api/v1/import/prometheus`表示把内部指标 Push 到`/api/v1/import/prometheus`中，参见[这个文档]({{< relref "../write/api.md#prometheus" >}})。`-pushmetrics.url`参数可以指定多个地址。这种情况下 metrics 会被 Push 到所有目标地址上。URL 中也可以包含上 Basic Auth 信息，格式是`http://user:pass@hostname/api/v1/import/prometheus`。Metrics 是被压缩后 Push 到`-pushmetrics.url`中的，请求头中带有`Content-Encoding: gzip`。这可以减少 Push 数据消耗的网络带宽。
+ `-pushmetrics.extraLabel`：在把 metrics 数据 Push 到`-pushmetrics.url`之前追加一些 Label 。每一个 Label 都是用`label="value"`的格式指定。启动参数`-pushmetrics.extraLabel`也是可以被多次指定的。这种情况下会将指定的多个 Label 全都追加到 metrics 数据中，再 Push 给`-pushmetrics.url`地址。
+ `-pushmetrics.interval`- Push 动作的间隔，默认是 10 秒一次。

例如，以下命令是让 VictoriaMetrics 将`/metrics`里的指标推送到`https://maas.victoriametrics.com/api/v1/import/prometheus`，并使用`user:pass`基本身份验证。在将指标发送到远程存储之前，会添加`instance="foobar"`和`job="vm"` Label 注入到所有的指标中：
...
### 参数调整？
+ 不需要调整 VictoriaMetrics 启动参数：它的默认启动参数都是很合理的，这些参数会自动根据可用的 CPU 和 RAM 资源进行调整。 
+ 不需要优化 OS 系统参数： VictoriaMetrics 已经针对默认的系统设置进行了优化。唯一一个可能需要调整的是增加操作系统中[打开文件数量的限制](https://medium.com/@muhammadtriwibowo/set-permanently-ulimit-n-open-files-in-ubuntu-4d61064429a)。这个建议不仅适用于 VictoriaMetrics，也适用于任何处理大量 HTTP 连接并将数据存储在磁盘上的服务。 
+ **VictoriaMetrics 是一个写入密集型应用程序，其性能取决于磁盘性能**。因此，请注意其他可能[占用磁盘资源](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/1521)的应用程序或工具（如[fstrim](http://manpages.ubuntu.com/manpages/bionic/man8/fstrim.8.html)）。 
+ 推荐使用`ext4`文件系统，并且[推荐在 GCP 上使用基于 HDD 硬盘的持久存储](https://cloud.google.com/compute/docs/disks/#pdspecs)，因为它内部自带复制机制，可以应对硬件故障，并且可以[动态调整大小](https://cloud.google.com/compute/docs/disks/add-persistent-disk#resize_pd)。如果你计划在`ext4`分区上存储超过`1TB`的数据或者计划将其扩展到`16TB`以上，则建议给`mkfs.ext4`设置以下选项：
...
VictoriaMetrics 可以为存储在`-storageDataPath`目录下的所有数据创建即时快照。访问`http://<victoriametrics-addr>:8428/snapshot/create`就可以创建即时快照。该接口将返回以下 JSON 响应：
...
快照放在`<-storageDataPath>/snapshots`目录下，其中`<-storageDataPath>`是启动参数。可以随时使用[vmbackup]({{< relref "../components/vmbackup.md" >}})将快照备份到外部存储中。

- `http://<victoriametrics-addr>:8428/snapshot/list`接口包含了可用快照列表。
- `http://<victoriametrics-addr>:8428/snapshot/delete?snapshot=<snapshot-name>`则可以删除`<snapshot-name>`快照.
- `http://<victoriametrics-addr>:8428/snapshot/delete_all`可以删除所有快照。
...
发送请求到`http://:8428/api/v1/admin/tsdb/delete_series?match[]=<timeseries_selector_for_delete>`，其中`<timeseries_selector_for_delete>`可以包含任何时间序列选择器。该

删除接口不支持指定时间范围，timeseries 只能完全删除。**已删除时间序列的存储空间不会立即释放**，他们会在后续的`part`合并过程中释放。

请注意，对于之前月份的数据可能永远不会触发后台合并，所以历史数据可能就无法释放存储空间。在这种情况下，[强制合并](#force-merge)可能有助于释放存储空间。

建议在实际删除指标之前使用`http://:8428/api/v1/series?match[]=<timeseries_selector_for_delete>`验证一下，避免删错。默认情况下，该查询仅扫描过去 5 分钟内的系列，所以你可能需要使用`start`和`end`参数才能得到匹配结果。

如果设置了`-deleteAuthKey`启动参数，则可以使用`authKey`保护`/api/v1/admin/tsdb/delete_series`接口，避免被误用。

Delete API 主要适用于以下情况：

一次性删除意外写入的无效（或不需要）时间序列。由于`GDPR`安全要求而一次性删除用户数据。 以下情况**不建议**使用 Delete API，因为它存在一定的系统开销：
- 定期清理不需要的数据。你只需防止不需要的数据持续写入 VictoriaMetrics 即可。可以通过[Relabeling](#relabeling)来实现。 
- 通过删除不需要的时间序列来减少磁盘空间使用情况。这种方法无法达到预期效果，因为已删除的时间序列会一直占用磁盘空间，直到下一次合并操作才会释放，所以有可能因为`part`永远不会发生合并操作而得不到释放。强制合并可用于释放由旧数据占用的磁盘空间。但请注意，VictoriaMetrics 不会从[倒排索引（也称为indexdb）]({{< relref "./dbengine.md#indexdb" >}})中删除这些时间序列。倒排索引创建后就无法改变，只有到了保存期限，才会随着`partition`清理而被清理。

最好使用`-retentionPeriod`启动参数以有效地清理旧数据。

### 强制合并 {#force-merge}
VictoriaMetrics 在[后台执行数据压缩](https://medium.com/@valyala/how-victoriametrics-makes-instant-snapshots-for-multi-terabyte-time-series-data-e1f3fb0e0282)，以此来保持在处理新数据时的依然能保持良好性能。
这些压缩（合并）是独立地针对每个月份分区`partition`进行的。这就意味着如果没有新数据写到`partition`，就会停止对月份`parition`的压缩。
有时需要触发旧`parition`的压缩，例如为了释放被[删除Timeseries](#delete-timeseries)占用的磁盘空间。
在这种情况下，可以通过请求`/internal/force_merge?partition_prefix=YYYY_MM`来启动指定的`partition`上的进行强制合并，其中`YYYY_MM`是`partition`名称。
例如，`http://victoriametrics:8428/internal/force_merge?partition_prefix=2020_08`将会启动2020年8月份的`parition`强制合并。
调用`/internal/force_merge`会立即返回，而真实的合并操作会在后台运行。
强制合并可能会消耗 CPU、磁盘IO和存储空间资源。在正常情况下没必要强制合并，因为当有新数据写入时，VictoriaMetrics 会自动在后台[选择最佳的时间和策略执行合并](https://medium.com/@valyala/how-victoriametrics-makes-instant-snapshots-for-multi-terabyte-time-series-data-e1f3fb0e0282)。

### 补数据 {#backfilling}
VictoriaMetrics 通过[多种写入方式]({{< relref "../write/api.md" >}})将任意时间的历史数据补写到 DB 中。

建议在写入历史数据时，使用`-search.disableCache`启动参数禁用查询缓存，因为缓存机制假设数据都是实时写入的，历史数据不会改变；如果你改变了，则被缓存的数据就脏了。
补写完成后再打开缓存，让`vmselect`缓存最新的历史数据块。
...
### 备份 {#backup}
VictoriaMetrics 支持使用 [vmbackup]({{< relref "../components/vmbackup.md" >}}) and [vmrestore]({{< relref "../components/vmrestore.md" >}}) 工具执行备份和恢复。

### 去重特性 {#deduplicate}
VictoriaMetrics 每个 timeseries 在`-dedup.minScrapeInterval`时间间隔内只保留一个具有最大时间戳的 raw sample。例如，`-dedup.minScrapeInterval=60s`表示一个 timeseries 在每60秒间隔内保留一个具有最大时间戳（最后写入）的原始样本。这与 [Prometheus 中的过期规则](https://prometheus.io/docs/prometheus/latest/querying/basics/#staleness)逻辑一样。

如果给定的`-dedup.minScrapeInterval`间隔中，存在多个具有相同时间戳的 raw sample，则保留 value 最大的数据。

请注意，要进行去重操作，原始样本的 Label 必须完全相同。例如，这就是为什么对 [vmagents 做高可用]({{< relref "../components/vmagent.md#high-availability" >}}) 时，配置需要一模一样的原因。

建议将`-dedup.minScrapeInterval`设置为 Prometheus 配置文件中`scrape_interval`的值。建议所有采集目标都使用一致的采集间隔，请参阅详细信息[文章](https://www.robustperception.io/keep-it-simple-scrape_interval-id)。

通过去重操作可以减少磁盘空间用量，特别是当多个配置完全相同的 vmagent 或 Prometheus 实例以 HA 形式成对部署，并写入数据到同一个 VictoriaMetrics 实例时，开启去重就更加有必要了。这些 vmagent 或 Prometheus 实例必须在其配置文件中具有完全相同的`external_labels`，以避免同一个目标的数据进入到 DB 中变成了不同的 timeseries。

建议为每个不同的 vmagent HA 实例使用不同的`-promscrape.cluster.name`参数，这样去重操作不会把多个 vmagent 自身的指标给合并掉。请参阅[详细文档]({{< relref "../components/vmagent.md#high-availability" >}})了解更多信息。

### 多租户 {#tenant}

单机版的 VictoriaMetrics 不支持多租户。[集群版本]({{< relref "./cluster.md#tenant" >}})支持。

### 多副本 {#replication}

单机版的 VictoriaMetrics 不支持实例级别的复制。可使用[集群版]({{< relref "../ops/cluster.md" >}})本代替。详细信息请参阅[这些文档]({{< relref "./cluster.md#replication" >}})。 
...
尽管单机版的 VictoriaMetrics 无法扩展到多个节点，但它在资源使用方面进行了优化，包括存储大小/带宽/IOPS、RAM 和 CPU。一个单机版的 VictoriaMetrics 可以在垂直方向上扩展，并替代其他解决方案（如 Thanos、Uber M3、InfluxDB 或 TimescaleDB）等规模集群。请参阅垂直扩展的[基准测试结果](https://medium.com/@valyala/measuring-vertical-scalability-for-time-series-databases-in-google-cloud-92550d78d8ae)。

**首先尝试使用单机版的 VictoriaMetrics，如果你仍然需要具有水平扩展的诉求，则可以切换到[集群版本]({{< relref "./cluster.md" >}})。**

### Relabeling {#relabeling}
VictoriaMetrics 支持对所有接收到的指标进行与 Prometheus 兼容的 Relabel，只需使用`-relabelConfig`参数指定一个包含`relabel_config`配置项的文件即可。`-relabelConfig`也可以指向http地址。例如，`-relabelConfig=https://config-server/relabel_config.yml`。 

以下文档可能对理解 Relabel 处理有所帮助：
...
`-relabelConfig`文件中可以使用特殊的占位符，格式为`%{ENV_VAR}`，它们将被替换成相应的环境变量。

`-relabelConfig`文件内容示例：

```yaml
# 增加 {cluster="dev"} label.
- target_label: cluster
  replacement: dev

# 丢掉带有 __meta_kubernetes_pod_container_init Label 的数据.
- action: drop
  source_labels: [__meta_kubernetes_pod_container_init]
  regex: true
```

VictoriaMetrics 扩展了 Relabel 功能，例如 Graphite 风格标注语法。有关更多详细信息，请参阅[这些文档]({{< relref "../components/vmagent.md#relabeling" >}})。
...
VictoriaMetrics 使用各种内部缓存。这些缓存在组件被优雅关闭时（例如通过发送`SIGINT`信号关闭 VictoriaMetrics）存储到`<-storageDataPath>/cache`目录中。下次启动 VictoriaMetrics 时会读取这些缓存。

如果需要在下次启动时删除此类缓存。可以通过以下方式完成：

+ 在 VictoriaMetrics 停止后手动删除`<-storageDataPath>/cache`目录。
+ 在重新启动 VictoriaMetrics 之前将`reset_cache_on_startup`文件放置在`<-storageDataPath>/cache`目录中。 在这种情况下，VictoriaMetrics 将自动在下次启动时删除所有缓存。有关详细信息，请参阅这个[issue](https://github.com/VictoriaMetrics/VictoriaMetrics/issues/1447)。

### Rollup 结果缓存 {#rollup cache}

VictoriaMetrics 默认缓存查询响应数据。这样就可以在`time`,`start`和`end`时间参数不断递增的场景下，提升对`/api/v1/query`和`/api/v1/query_range`的重复查询性能。（比如，一个 Grafana 页面定时刷新，会反复发送重复请求，其中只有`time`,`start`和`end`参数随着时间变化）

如果有历史数据补写到 DB 中，该缓存机制就会导致 VictoriaMetrics 返回不正确的结果。详情请参见[这些文档](#backfilling)。

可以通过启动参数`-search.disableCache`禁用 VictoriaMetrics 的全局 rollup 缓存，也可以在请求`/api/v1/query`和`/api/v1/query_range`接口时带上`nocache=1`参数表示不使用该缓存。。
...
单机版和集群版的 Grafana 监控面板都包含了缓存部分，将缓存指标的可视化。面板显示了各种类型缓存的当前内存使用情况，以及缓存命中率。如果命中率接近100%，则表示缓存效率已经非常高，不需要进行任何调整。
在故障排除部分的面板中，`Cache usage %`显示了各类型缓存大小与缓存限制之间的百分比。如果百分比低于100%，则无需进一步调整。

请注意，默认缓存大小已根据最实际的场景和运行环境进行了精心调整。只有在你对这些参数有深入的理解，并且`vmstorage`机器上还有比较充足的空闲内存，才需要进行调整。

要覆盖默认值，请参阅带有`-storage.cacheSize`前缀的参数。可以在[此处](#flags)查看所有启动参数的完整描述。


### 基数限制 {#cardinality}

默认情况下，VictoriaMetrics 不限制存储的时间序列数量。可以通过设置以下启动参数来增加限制：

+ `-storage.maxHourlySeries`- 限制在一个小时内可以新增的时间序列数量。对于限制[活动时间序列]({{< relref "../faq.md#what-is-an-active-time-series" >}})的数量非常有用。
+ `-storage.maxDailySeries`- 限制一天内可以新增的时间序列数量。对于限制每日[替换率]({{< relref "../faq.md#what-is-high-churn-rate" >}})非常有用。

可以同时设置这两个限制。如果达到任何一个限制，那么新时间序列的输入样本将被丢弃。被丢弃的系列样本会以`WARN`级别记录在日志中。

超出限制的情况可以通过以下指标进行监控：

+ `vm_hourly_series_limit_rows_dropped_total`- 由于超过小时级限制的独立时间序列数量而被丢弃指标数量。
+ `vm_hourly_series_limit_max_series`- 小时级限制，通过`-storage.maxHourlySeries`设置的值。

`vm_hourly_series_limit_current_series`- 过去一小时内 timeseries 的数量。当过去一小时内 timeseries 的数量超过`-storage.maxHourlySeries`的`90%`时，需要及时关注。

+ `vm_daily_series_limit_rows_dropped_total`- 由于超过每日独立时间序列数量限制，被丢弃指标数量。
+ `vm_daily_series_limit_max_series`- 天级限制，通过`-storage.maxDailySeries`设置的值。

`vm_daily_series_limit_current_series`- 在过去的一天中，timeseries 数量。当 timeseires 在过去的一天内超过`-storage.maxDailySeries`的`90%`时，需要及时关注。

这些限制是近似值，大约有`1%`的误差；为了降低限制逻辑的复杂性，内部用布隆过滤器实现，因此有点误差。

更多进阶内容参见 [vmagent 的基数限制]({{< relref "../components/vmagent.md#cardinality-limiter" >}})

### 查询追踪 {#trace}

VictoriaMetrics 支持查询追踪，可用于定位查询处理过程中的瓶颈。有点类似于 Postgresql 的`EXPLAIN ANALYZE`。

可以通过`trace=1`查询参数来启用特定查询请求的追踪。在这种情况下，VictoriaMetrics 将查询最终放入 Response JSON 的数据中的 trace 字段里。
...
所有追踪中的耗时和时间戳都以毫秒为单位。查询追踪的特性默认是开启的。可以通过在 VictoriaMetrics 上传递`-denyQueryTracing`参数来关闭它。

[VMUI]({{< relref "../components/vmui.md" >}}) 提供了一个 UI 界面:

+ 对于 query 追踪：只需要选中`Trace query`复选框，然后重新跑一下查询语句就可以得到执行 Trace。
+ 对于探索自定义追踪：进入`Trace analyzer`页面，然后上传或粘贴其他系统返回的 trace JSON 数据。

### 安全 {#security}
一般安全建议：

+ 所有的 VictoriaMetrics 组件必须在受保护的私有网络中运行，不能要暴露到不可信任的网络（如互联网）中。
+ 来自不可信任网络到达 VictoriaMetrics 组件的所有请求都必须通过认证代理（例如 vmauth 或 vmgateway）进行。代理必须设置适当的鉴权。
+ 在配置位于 VictoriaMetrics 组件前面的认证代理时，选择性地对外开放有必要的 API 接口列表，不要全部开放。

VictoriaMetrics 提供了下面这些启动参数，可以提升系统的安全性：

+ `-tls`, `-tlsCertFile`和`-tlsKeyFile`用来开启 HTTPS.
+ `-httpAuth.username`和`-httpAuth.password`使用 [HTTP Basic Authentication](https://en.wikipedia.org/wiki/Basic_access_authentication) 来保护所有的 HTTP 接口。
+ `-deleteAuthKey`用来保护`/api/v1/admin/tsdb/delete_series`接口。参见[如何删除 timeseries](#delete-timeseries).
+ `-snapshotAuthKey`用来保护`/snapshot*`一系列接口。参见[快照文档](#snapshot).
+ `-forceMergeAuthKey`用来保护`/internal/force_merge`接口。参见 [强制合并文档](#force-merge).
+ `-search.resetCacheAuthKey`用来保护`/internal/resetRollupResultCache`接口。 更多详情参见[补数据文档](#backfilling)。
+ `-configAuthKey`用来保护`/config`接口，因为它可能包含一些敏感的信息，比如密码。
+ `-flagsAuthKey`用来保护`/flags`接口。
+ `-pprofAuthKey`用来保护`/debug/pprof/*`接口，这是用来做性能分析。
+ `-denyQueryTracing`用来禁用 [查询追踪](#trace).

专门针对 Graphite 和 OpenTSDB 格式数据写入设置 TCP 和 UDP 内部地址。例如，将`-graphiteListenAddr=:2003`改为`-graphiteListenAddr=<internal_iface_ip>:2003`。这样可以防止来自不受信任的网络接口的非法请求。
...
+ 建议使用默认启动参数，不要自己去设置它。
+ 建议在故障排除过程中检查日志，因为日志很有用，里面包含很多有用的信息。
+ 建议至少预留`50%`的 CPU、磁盘IO和 RAM 资源作为备用，这样 VictoriaMetrics 就可以应对突发流量，不出现性能问题。
+ VictoriaMetrics 需要足够空闲磁盘空间将数据文件合并成更大的文件。当没有足够的剩余空间时，它可能会变慢（因为数据文件无法被合并）。因此，请确保`-storageDataPath`目录至少有`20%`的空闲空间。当前剩余可用空间量可以通过`vm_free_disk_space_bytes`指标进行监控。
存储在磁盘上的数据总大小可以通过`vm_data_size_bytes`指标进行监控。还可以查看`vm_merge_need_free_disk_space`指标，如果由于没有足够磁盘空间而导致无法启动合并，则该指标值会大于`0`。
该值显示的是受磁盘空间影响的`partition`数量，当磁盘空间足够空闲后，后台合并会自动恢复执行过。
+ VictoriaMetrics 会将写入数据缓冲到内存中，并在几秒钟后将其刷新到磁盘中。这可能会导致以下*问题*：
  - 数据写入后要等几秒数据才能被查到。可以通过请求`/internal/force_flush`接口将内存缓冲区数据刷盘，这样就能被查到了。*此接口主要用于测试和调试目的*。
  - 在非正常关闭（即OOM、`kill -9`或硬件故障）时，最后几秒钟写入的数据可能会丢失。`-inmemoryDataFlushInterval`参数可以控制将内存中的数据刷盘的频率。
+ 如果 VictoriaMetrics 性能变差，并且每秒写入`100K`个数据点占用超过一个 CPU 核，则很可能是内存不够了，不足以支撑[活动时间序列]({{< relref "../faq.md#what-is-active-timeseries" >}})。
VictoriaMetrics 有`vm_slow_*`指标，例如`vm_slow_row_inserts_total`和`vm_slow_metric_name_loads_total`，它们可以用于判断内存是否够用。建议增加 VictoriaMetrics 的内存来改善性能。
+ 如果同一 Metric 的 Label 顺序如果随时间有变化（例如`metric{k1="v1",k2="v2"}`可能变为`metric{k2="v2",k1="v1"}`），则建议使用`-sortLabels`参数运行 VictoriaMetrics，可以减少内存使用和CPU使用率，因为不同的顺序对 VictoriaMetrics 来说属于不同的 timeseries。
+ VictoriaMetrics 优先考虑数据写入而不是数据查询。因此，如果没有足够的资源进行数据写入，则数据查询可能会明显变慢。
+ 如果 VictoriaMetrics 由于磁盘错误导致某些`part`文件损坏而无法工作，则只需删除带有损坏`part`的目录即可。在 VictoriaMetrics 未运行时，删除`<-storageDataPath>/data/{big,small}/YYYY_MM`目录下的子目录可以恢复 VictoriaMetrics，但损坏文件里的数据就丢失了。
+ 如果你在图表上看到断点，请尝试通过向`/internal/resetRollupResultCache`发送请求来重置缓存。如果这样可以消除图表上的空白断点间隙，则很可能是早于`-search.cacheTimestampOffset`的数据出现了更新(补写)。
+ 如果你是从 InfluxDB 或 TimescaleDB 切换过来的，可能需要使用`-search.setLookbackToStep`参数。这将避免 VictoriaMetrics 使用的默认间隙填充算法。默认情况下，它假设每个时间序列是连续的而非离散的，因此会用固定间隔填补 raw sample 之间的空白。
+ 通过 [/api/v1/status/tsdb]({{< relref "../query/api.md#apiv1statustsdb-tsdb-stats" >}}) 接口可以定位导致高基数或高替换率的 Metric 和 Label。
+ 如果要在 VictoriaMetrics 中记录新时间序列，请传递`-logNewSeries`启动参数。
+ VictoriaMetrics 通过`-maxLabelsPerTimeseries`参数限制每个 Metric 的 Label 数量。这可以防止写入指标包含了太多 Label。建议观察`vm_metrics_with_dropped_labels_total`指标，看是否需要根据线上实际情况调整`-maxLabelsPerTimeseries`。
+ 如果你在 VictoriaMetrics 中存储 Graphite 指标（如`foo.bar.baz`），则可以使用`{__graphite__="foo.*.baz"}`过滤器选择此类指标。详细信息请参阅[相关文档]({{< relref "../query/metricsql/_index.md#graphite-filter" >}})。
+ 在数据写入期间，VictoriaMetrics 会忽略`NaN`值。
